...
- **KRC-721 NFT Certificates**: Implements non-custodial minting flow, adhering to KRC-721 standard. Uses IPNS (mutable IPFS pointers via W3Name) for dynamic per-user certificate metadata. Collection buri points to `ipns://KEY/` which resolves to IPFS folder containing token metadata files. Mainnet indexer: https://kaspa-krc721d.kaspa.com/ (KaspacomDAGs). Fee requirements: Deploy = 1,000 KAS minimum, Mint = 10 KAS minimum.
