...
// - Users whitelisted via "discount" operation pay 10 KAS (PoW fee only, discountFee=0)
...
class KRC721Service {
  ...
  private async loadKaspaModule(): Promise<void> {
    ...
    console.log("[KRC721] Searching for WASM in paths:", pathCandidates);
    
    let wasmPath: string | null = null;
    for (const candidate of pathCandidates) {
      if (fs.existsSync(candidate)) {
        wasmPath = candidate;
        console.log(`[KRC721] Found WASM at: ${candidate}`);
        break;
      }
    }
    
    if (!wasmPath) {
      console.error("[KRC721] WASM not found in any candidate paths");
      console.error("[KRC721] CWD:", process.cwd());
  ...
  async deployCollection(imageUrl: string, buriUrl?: string): Promise<DeployResult> {
    ...
    try {
      ...
      // - Whitelisted users pay discountFee (10 KAS) instead of royaltyFee
